...
export function RunDetailPage() {
  ...
  // finished run) OR stage lifecycle event (stage.started / stage.finalized)
  // refetches the run so attempts and verdicts appear without a manual reload.
  // Debounced so an event burst causes one refetch.
  // NOTE: do NOT use overlay.version here — budget.tick bumps it every tick.
  const qc = useQueryClient();
  const gateEventCount = overlay.gateEvents?.length ?? 0;
  const stageLifecycleCount = overlay.stageLifecycleCount ?? 0;
  useEffect(() => {
    if (!runId || (gateEventCount === 0 && stageLifecycleCount === 0)) return;
    const t = setTimeout(() => void qc.invalidateQueries({ queryKey: qk.run(runId) }), 400);
    return () => clearTimeout(t);
  }, [gateEventCount, stageLifecycleCount, qc, runId]);
